...
def MAIN( menu ):
    ...
    for (mode, url, name) in menu:
        addon.add_directory({'mode': mode, 'url': url, 'name': name.title()},{'title': name.title()})
    ...
    if r:
        for name, url, year, quality in r:
            ...
            if meta['trailer']:
                ...
                contextmenu_items= []
                meta['title'] = name+' '+quality
            contextmenu_items.append(['[COLOR lime][B]A[/B]dd [B]T[/B]o [B]L[/B]ibrary[/COLOR]','XBMC.RunPlugin(%s)' %
                                      addon.build_plugin_url({'mode': 'add2lib', 'name': name, 'url': baseUrl+url, 'img': meta['cover_url'],
                                                              'fanart':meta['backdrop_url'], 'infol': '', 'year': year})])
...
def add2lib( url, name, infol, year ):
    addon.log('Add To Library %s , %s, %s' % (name,year,url))

    path = xbmc.translatePath( addon.get_setting('movie-folder') )
    string = addon.build_plugin_url({'mode': 'playStreamUrl', 'url': url,
                                     'name': name, 'infol': ''})
    filename = '%s.strm' % name
    path = xbmc.makeLegalFilename( os.path.join( path, name, filename ))
    ...
    if xbmcvfs.exists(path):
        print 'already added'
        addon.show_small_popup( addon.get_name()+' adding to library'.title(), name+' Already added'.title(), 5000, addon.get_icon())
        return
    
    addon.show_small_popup( addon.get_name()+' adding to library'.title(), name.title(), 5000, addon.get_icon())
...
def playStreamUrl( url, infol, name ):
    ...
    if len(qt) >1:
        ...
        if auto_play == 'false':
            ...
            try:
                ...
                addon.show_small_popup( addon.get_name()+' Something went wrong'.title(),
                                        'Playing '+name+', in lower quality'.title(), 5000, addon.get_icon())
    ...
    if r:
        r = re.search(r'(https\:\/\/redirector\.googlevideo.*?)\'',str(r.headers), re.I)
        if r:
            videourl = r.group(1)
...
def setView(content, viewType):
    addon.log('viewtype %s, %s'% (content, viewType))
    print addon.get_setting(viewType)
    print addon.get_setting('auto-view')
    if content:
